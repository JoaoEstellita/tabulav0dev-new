// Lógica pura do WhatsApp (DDI + número → E.164). Separada do componente para
// ser testável sem React Native. Ver src/components/WhatsAppInput.tsx.
...
/** Quebra um número guardado (dígitos com código, "+..." ou vazio) em DDI + local. */
...
/** Monta o E.164 com "+": `+<ddi><local>` (só dígitos). Vazio se sem número local. */
export function toE164(ddi: string, local: string): string {
  const dd = String(ddi || "").replace(/\D/g, "")
  const ll = String(local || "").replace(/\D/g, "")
  return ll ? `+${dd}${ll}` : ""
}
