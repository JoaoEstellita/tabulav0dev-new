...
class AstrologyCacheService {
  ...
  private readonly DATA_VERSION = '1.5'
  ...
  async getCache(userId: string): Promise<AstrologyCache | null> {
    try {
      ...
      console.log('������ Buscando cache no Firestore...')
      ...
      if (cacheDoc.exists()) {
        ...
        console.log('ԣ� Cache do Firestore carregado e salvo localmente')
        return cache
      }
  ...
  async saveCache(
    ...
    try {
      ...
      console.log('ԣ� Valida+�+�o passou - nenhum campo undefined encontrado')
      ...
      console.log(`��ƥ Cache salvo - Fonte: ${source}, Requests hoje: ${dailyRequestCount}/${this.MAX_DAILY_REQUESTS}`)
    } catch (error) {
      console.warn('��ᴩ� N+�o foi poss+�vel salvar cache no Firestore, salvando apenas localmente:', error)
  ...
  async clearCache(userId: string): Promise<void> {
    try {
      ...
      console.log('���洩� Cache limpo com sucesso')
    } catch (error) {
  ...
  async forceRefresh(userId: string): Promise<void> {
    console.log('���� For+�ando refresh do cache (modo administrativo)')
